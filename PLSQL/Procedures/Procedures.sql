...
BEGIN
    ...
    FOR R_SALDOS IN (
        ...
        IF ((R_SALDOS.DIA_OPORTUNO < LN_DIA_ACTUAL
        AND R_SALDOS.PERIODO_MES_CUENTA = LN_MES_ACTUAL
        AND R_SALDOS.PERIODO_ANIO_CUENTA = LN_ANIO_ACTUAL)
        OR (R_SALDOS.PERIODO_MES_CUENTA < LN_MES_ACTUAL
        AND R_SALDOS.PERIODO_ANIO_CUENTA = LN_ANIO_ACTUAL)) AND R_SALDOS.PERIODO_MES_CUENTA <= LN_MES_ACTUAL AND R_SALDOS.VALOR_DESCUENTO = 0 AND R_SALDOS.VALOR_MORA > 0 THEN
            NULL;
        ELSIF ((R_SALDOS.DIA_OPORTUNO < LN_DIA_ACTUAL
        AND R_SALDOS.PERIODO_MES_CUENTA = LN_MES_ACTUAL
        AND R_SALDOS.PERIODO_ANIO_CUENTA = LN_ANIO_ACTUAL)
        OR (R_SALDOS.PERIODO_MES_CUENTA < LN_MES_ACTUAL
        AND R_SALDOS.PERIODO_ANIO_CUENTA = LN_ANIO_ACTUAL)) AND R_SALDOS.VALOR_DESCUENTO > 0 AND R_SALDOS.VALOR_MORA = 0 THEN
        ...
        AND R_SALDOS.PERIODO_ANIO_CUENTA = LN_ANIO_ACTUAL)) AND R_SALDOS.VALOR_DESCUENTO = 0 AND R_SALDOS.VALOR_MORA = 0 THEN
            ...
            WHERE
                COD_CUENTA_COBRO = R_SALDOS.COD_CUENTA_COBRO;
            COMMIT;
        ELSIF R_SALDOS.DIA_OPORTUNO >= LN_DIA_ACTUAL AND R_SALDOS.PERIODO_MES_CUENTA >= LN_MES_ACTUAL AND R_SALDOS.PERIODO_ANIO_CUENTA >= LN_ANIO_ACTUAL AND R_SALDOS.VALOR_DESCUENTO = 0 AND R_SALDOS.VALOR_MORA = 0 THEN
