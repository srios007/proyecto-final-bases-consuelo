...
------------------------------------------------------ Procedimiento que inserta en la tabla de auditoria los valores correspondientes
CREATE OR REPLACE PROCEDURE PR_AUDITORIA_PAGO (
    PK_CUENTA_COBRO IN CUENTA_COBRO.COD_CUENTA_COBRO%TYPE,
    PK_PAGO IN PAGO.COD_PAGO%TYPE,
    PI_CUENTA IN CUENTA_COBRO.ESTADO_CUENTA%TYPE,
    PV_PAGADO IN PAGO.VALOR_PAGADO%TYPE,
    PC_ERROR OUT INTEGER,
    PM_ERROR OUT VARCHAR
) AS
    LK_PAGO   PAGO.COD_PAGO%TYPE := PK_PAGO;
    LI_CUENTA CUENTA_COBRO.ESTADO_CUENTA%TYPE := PI_CUENTA;
    LV_PAGADO PAGO.VALOR_PAGADO%TYPE := PV_PAGADO;
    LK_AUDIT  AUDIT_CUENTAS.COD_AUDIT_CUENTAS%TYPE;
    LF_ACTUAL AUDIT_CUENTAS.FECHA_MODIFICACION%TYPE;
    L_USUARIO AUDIT_CUENTAS.USUARIO%TYPE;
BEGIN
    DBMS_OUTPUT.PUT_LINE(LK_PAGO);
    DBMS_OUTPUT.PUT_LINE(LI_CUENTA);
    DBMS_OUTPUT.PUT_LINE(LV_PAGADO);
    SELECT
    ...
    ELSIF LK_PAGO = 0 OR LV_PAGADO = 0 THEN
        INSERT INTO AUDIT_CUENTAS (
            COD_AUDIT_CUENTAS,
            COD_CUENTA_COBRO,
            COD_PAGO,
...
EXCEPTION
    WHEN OTHERS THEN
        PC_ERROR := 1;
        PM_ERROR := 'Error al auditar el pago / cuenta de cobro';
        RAISE_APPLICATION_ERROR(-20001, 'PR_AUDITORIA_PAGO Ha ocurrido un error: '
            || SQLCODE
            || SQLERRM);
END PR_AUDITORIA_PAGO;
/